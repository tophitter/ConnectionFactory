<?php
    ...
    class EnumMap extends SplObjectStorage {
        ...
        public function setFlags($flags)
        {
            ...
            $keyFlag = $flags & 7;
            if ($keyFlag > 4)
                throw new InvalidArgumentException('Unsupported flag given for key() behavior');
            elseif (!$keyFlag)
                $keyFlag = $this->flags & 7;

            $currentFlag = $flags & 56;
            if ($currentFlag > 40)
                throw new InvalidArgumentException('Unsupported flag given for current() behavior');
            elseif (!$currentFlag)
         ...
         * @see contains()
         */
        public function offsetExists($enumerator) { return $this->contains($enumerator); }
